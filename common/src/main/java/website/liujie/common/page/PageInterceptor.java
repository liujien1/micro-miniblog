...
public class PageInterceptor implements Interceptor {
    ...
    private static final ThreadLocal<PageModel> PAGE_CONTEXT = new ThreadLocal<PageModel>();
    private String databaseType;//数据库类型，不同的数据库有不同的分页方法

    public static final String PAGE_KEY = "page";//别名
    /**
     * 拦截后要执行的方法
     */
    public Object intercept(Invocation invocation) throws Throwable {
        ...
        if (obj instanceof HashMap) {
            ...
            if(map.containsKey(PAGE_KEY)){	//2014-12-22 加入逻辑判断是否存在  PAGE_KEY 对象
                Object page = map.get(PAGE_KEY) ;
                if(  page  instanceof  PageModel ) {
                        /*PageModel<?> p = (PageModel<?>) page;
                        ...
                        //获取分页Sql语句
                        String pageSql = this.getPageSql(p, sql);
                        set(p);
                        ...
                        PageModel p = (PageModel) page;
                        if (p != null) {
                            p.setTotalSize(queryTotal(invocation, mappedStatement, boundSql, obj, sql));
                            set(p);
     ...
     * @return
     */
    private String pageSql(String sql, PageModel page) {
        if (page == null ) {
            return sql;
        }
        ...
        sb.append(sql);
        sb.append(" limit " + (page.getPageIndex()-1)*page.getPageSize() + ","
     ...
     * @param p
     */
    private static void set(PageModel p) {
        PAGE_CONTEXT.set(p);
    }
    ...
    private void setTotalRecord(PageModel<?> page,
        ...
        ParameterHandler parameterHandler = new DefaultParameterHandler(mappedStatement, page.getBeanParams(), countBoundSql);
        ...
        try {
            ...
            if (rs.next()) {
                ...
                page.setTotalPage(totalRecord/page.getPageSize()+1);
            }
    ...
    private String getCountSql(String sql) {
        ...
        int endIndex=sql.indexOf(";")!=-1?sql.indexOf(";")+1:sql.length();
        return "SELECT COUNT(0) " + sql.substring(beginIndex,endIndex);
